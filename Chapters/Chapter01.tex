...
\bibitem{DellLatitude3190} Dell Latitude 3190, 8GB RAM.
\bibitem{EquityViolationData} These data demonstrate the educational equity violation created by inadequate hardware.
\bibitem{ScreenreaderLagImpact} Any lag in screenreader responsiveness of >1 sec means the student is behind their peers and their educational opportunity is limited by inadequate technology accommodation.
\bibitem{DellPrecision3530} Dell Precision 3530, 16GB RAM.
\bibitem{LenovoThinkPadE16} Lenovo ThinkPad E16, 24GB RAM.
\bibitem{MicrosoftSurface3} Microsoft Surface 3, 32GB RAM.
\bibitem{ImmediateResponseEquity} 0.01 represents an immediate response approaching educational equity.
\bibitem{FrameworkLaptop16} Framework 16.0, 64GB RAM.
\bibitem{TrueEquityStandard} Achieves true educational equity standard.
\bibitem{APHQuotaFunds} Typically purchased via APH quota funds.
\bibitem{EquityAnalysisRevision} Revised based on equity analysis.
\bibitem{RequiresJAWSFusion} Requires JAWS or Fusion.
\bibitem{WindowsBuiltInScreenreader} Windows built-in screenreader.
\bibitem{FreePremiumVoices} Free, but premium voices cost \$70.
\bibitem{PricingChange2024} Pricing changed October 2024.
\bibitem{WindowsBuiltInMagnifier} Windows built-in magnifier.
\bibitem{EquityViolationAccessibility} EQUITY VIOLATION - Inadequate for accessibility.
\bibitem{UnacceptableEquityViolation} UNACCEPTABLY INADEQUATE - Violates equity standard.
\bibitem{ApproachesEquityAcceptable} APPROACHES EQUITY - Acceptable performance.
\bibitem{AchievesEquityCompliance} ACHIEVES EQUITY - True accessibility compliance.
\end{thebibliography}
